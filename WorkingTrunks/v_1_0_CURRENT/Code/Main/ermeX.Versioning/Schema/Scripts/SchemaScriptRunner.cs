...
namespace ermeX.Versioning.Schema.Scripts
{
    internal abstract class SchemaScriptRunner
    {
        ...
        private QueryHelper _dbQueryHelper;

        protected SchemaScriptRunner(DbEngineType engineType, IList<DataSchemaType> schemasApplied,
        ...
        private QueryHelper DbQueryHelper
        {
            get
            {
                return _dbQueryHelper ??
                       (_dbQueryHelper =
                        QueryHelper.GetHelper(EngineType, ConnectionString));
            }
        }
                                                   ...
                                                   DbEngineType configurationSourceType)
        {
            ...
            switch (configurationSourceType)
            {
                case DbEngineType.SqlServer2008:
                    result = new SqlServerSchemaScriptRunner(schemasApplied, configurationConnectionString);
                    break;
                case DbEngineType.Sqlite:
                case DbEngineType.SqliteInMemory:
                    result = new SqliteSchemaScriptRunner(schemasApplied, configurationConnectionString);
                    break;
                default:
