...
namespace ermeX.Tests.Acceptance
{
	...
	internal sealed class HandshakeTester : AcceptanceTester
	{
		...
		private void AssertDefaultServicesAreDefined(DbEngineType engineType, TestComponent targetComponent,
		                                             List<TestComponent> otherComponents)
		{
			var operationIds =
			...
			foreach (var otherComponent in otherComponents)
			{
				...
				{
					ServiceDetails suscription =
						suscriptions.SingleOrDefault(
							x => x.Publisher == otherComponent.ComponentId && x.OperationIdentifier == operationId);
		...
		private void AssertOutgoingSuscriptionsAreDefined(DbEngineType engineType, TestComponent targetComponent,
		                                                  List<TestComponent> otherComponents)
		{
			//filter definitions by component
			var suscriptions =
			...
			Assert.IsTrue(suscriptions.Count == expected,
			              string.Format("Expected: {0} Actual: {1}", expected, suscriptions.Count));
		...
		private void AssertIncommingSuscriptionsAreDefined(DbEngineType engineType, TestComponent targetComponent,
		                                                   List<TestComponent> otherComponents)
		{
			//filter definitions by component
			var suscriptions =
		...
		private void AssertConnectivityDetailsAreDefined(DbEngineType engineType, TestComponent targetComponent,
		                                                 List<TestComponent> otherComponents)
		{
			//filter definitions by component
			var components =
				GetDataHelper(engineType)
					.GetObjectsFromDb<ConnectivityDetails>()
					.Where(x => x.ComponentOwner == targetComponent.ComponentId)
					.ToList();
		...
		private void AssertComponentsAreDefined(DbEngineType engineType, TestComponent targetComponent,
		                                        List<TestComponent> otherComponents)
		{
			//filter definitions by component
			var components =
				GetDataHelper(engineType)
					.GetObjectsFromDb<AppComponent>("Components")
					.Where(x => x.ComponentOwner == targetComponent.ComponentId)
					.ToList();
