	...
	internal class RegisteringServicesTests : RegisteringTestsBase
	{
		...
		private void DoTestRegisterService(DbEngineType dbEngine)
		{
			...
			List<ServiceDetails> services =
				GetDataHelper(dbEngine)
					.GetList<ServiceDetails>()
					.Where(x => methods.Select(y => y.Name).Contains(x.ServiceImplementationMethodName))
					.ToList();
		...
		private void DoRepeatedServiceRegistrationTest<TService>(DbEngineType dbEngine, bool asSystemService)
