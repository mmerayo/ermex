...
namespace ermeX.Tests.DAL.Integration.DataSources
{
		...
		UpdatableByExternalComponentsTester<Repository<OutgoingMessageSuscription>, OutgoingMessageSuscription>
	{
		...
		protected override int InsertRecord(DbEngineType engine)
		{
			...
			dataAccessTestHelper.InsertAppComponent(_componentId, LocalComponentId, 0, false);

			return dataAccessTestHelper.InsertOutgoingMessageSuscriptions(_messageType, _updateTime, _componentId,
			                                                              LocalComponentId);
		}
